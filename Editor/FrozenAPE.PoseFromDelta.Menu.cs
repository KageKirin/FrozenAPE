...
namespace FrozenAPE
{
    public class FrozenAPE_PoseFromDeltaMenu : EditorWindow
    {
        ...
        [MenuItem("FrozenAPE/Pose in WORLD SPACE from ΔJSON...")]
        [MenuItem("GameObject/FrozenAPE/Pose in WORLD SPACE from ΔJSON...")]
        // [MenuItem("Assets/FrozenAPE/Pose in WORLD SPACE from ΔJSON...")] //< assets are readonly
        public static void PoseInWorldSpaceFromDeltaJson(MenuCommand menuCommand)
        {
            if (Selection.activeObject == null)
            {
                Debug.LogError("Nothing selected! Please select a GameObject.");
            ...
            IRigPuppeteer rigPuppeteer = new RigPuppeteer();
            rigPuppeteer.PoseFromDeltaInWorldSpace(go.GetComponentsInChildren<Transform>(true), posedBones.bones);
        }
    }
}
